...
internal sealed class VersionCheckBackupService
{
    private readonly VersionCheckService _analysis;
    private readonly VersionCheckRepairService _repairService;
    private readonly ILogger _logger;

    public VersionCheckBackupService(VersionCheckService analysis, VersionCheckRepairService repairService, ILogger logger)
    {
        _analysis = analysis;
    ...
    /// <summary>
    /// 恢复单个备份的核心逻辑（调用方需持有 <see cref="VersionCheckShared.RepairGate"/>）。
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            await VersionCheckFileOps.CopyFileDurablyAsync(entry.BackupPath, temporaryPath, cancellationToken);

            var stagedAnalysis = await _analysis.AnalyzeSinglePatchFileStructureAsync(
            ...
            var restoredHash = await VersionCheckFileOps.ComputeSha256Async(originalPath, cancellationToken);
            ...
            {
                await VersionCheckFileOps.TryWriteBackupMetadataAsync(
                    modDirectory,
                    rollbackPath,
                    originalPath,
                    ModBackupRepairKind.PreRestore,
                    0,
                    sourceBackupFileName: entry.BackupFileName,
                    logger: _logger,
                    cancellationToken: cancellationToken);
            }
        ...
        finally
        {
            if (temporaryPath is not null)
                VersionCheckFileOps.TryDeleteFile(temporaryPath);
        }
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            var metadata = await VersionCheckFileOps.ReadBackupMetadataAsync(backupFile.FullName, cancellationToken);
            ...
            var backupHash = await VersionCheckFileOps.ComputeSha256Async(backupFile.FullName, cancellationToken);
            ...
            var currentHash = currentExists
                ? await VersionCheckFileOps.ComputeSha256Async(originalPath, cancellationToken)
                : string.Empty;
            var analysis = await _analysis.AnalyzeSinglePatchFileStructureAsync(
                backupFile,
        ...
        PatchFileAnalysis analysis)
    {
        ...
        await _repairService.InspectPatchForRepairsAsync(backupFile, actions, blockers, companionSource);
