...
internal sealed class GameCompanionRecoveryReader
{
    ...
    public GameCompanionRecoveryReader(GameUnitReferenceReader unitReader, VersionCheckRepairService repairService, ILogger logger)
    {
    ...
    internal sealed record GameCompanionLocator(
        ...
        VersionCheckService.PatchTocEntry Entry);
    ...
    {
        var dataDirectory = _unitReader.GetConfiguredGameDataDirectory();
        if (dataDirectory is null)
            return null;

        var blockers = new List<string>();
        var entries = await _repairService.ReadAssistedPatchEntriesAsync(patchFile, blockers);
        ...
        await using var patchStream = VersionCheckService.OpenPatchReadStream(patchFile);
        foreach (var neededEntry in needed)
        {
            cancellationToken.ThrowIfCancellationRequested();
            var toc = neededEntry.Toc;
            if (toc.TocSize > GameUnitReferenceReader.MaxBundleResourceBytes ||
            ...
            if (!await VersionCheckService.ReadAtAsync(patchStream, checked((long)toc.TocOffset), modMainData))
                return null;
            ...
            foreach (var candidate in candidates)
            {
                ...
                var gameMainData = TryReadPackageRange(
                    ...
                    GameUnitReferenceReader.MaxBundleResourceBytes);
    ...
    private GameCompanionLookup BuildGameCompanionLookup(
        ...
        CancellationToken cancellationToken)
    {
        var bundleIndexData = GameUnitReferenceReader.DecodeDsarFile(Path.Combine(dataDirectory.FullName, "bundles.nxa"));
        ...
        for (var i = 0; i < bundles.Length; i++)
            bundles[i] = GameUnitReferenceReader.LoadBundleInfo(Path.Combine(dataDirectory.FullName, $"bundles.{i:00}.nxa"));
        ...
        for (var packageIndex = 0; packageIndex < packageCount; packageIndex++)
        {
            ...
            var name = GameUnitReferenceReader.ReadNullTerminatedString(bundleIndexData, nameOffset);
    ...
    internal sealed class GameCompanionLookup(
        GameUnitReferenceReader.BundleInfo[] bundles,
        Dictionary<(long FileId, long TypeId), List<GameCompanionLocator>> resources)
    {
        public GameUnitReferenceReader.BundleInfo[] Bundles { get; } = bundles;
    ...
    private static GameUnitReferenceReader.PackageItem[]? ReadRecoveryPackageItems(
        ...
        Dictionary<(long FileId, long TypeId), List<GameCompanionLocator>> result)
    {
        ...
        for (var index = 0; index < numFiles; index++)
        {
            ...
            var entry = new VersionCheckService.PatchTocEntry(
        ...
        bool readWholeResource = false)
    {
        ...
        try
        {
            ...
            var data = GameUnitReferenceReader.ReadBundleResource(bundles[item.BundleIndex], bundleOffset, maxBytes);
    ...
    private static ulong GetCompanionOffset(VersionCheckService.PatchTocEntry entry, string suffix) =>
        suffix.Equals(".gpu_resources", StringComparison.OrdinalIgnoreCase)
            ? entry.GpuOffset
            : entry.StreamOffset;

    private static uint GetCompanionSize(VersionCheckService.PatchTocEntry entry, string suffix) =>
        suffix.Equals(".gpu_resources", StringComparison.OrdinalIgnoreCase)
            ? entry.GpuSize
            : entry.StreamSize;
}
