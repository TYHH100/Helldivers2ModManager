...
internal sealed partial class VersionCheckRepairService
{
        ...
        Func<long, AssistedLodStrategy> lodStrategySelector)
    {
        ...
        var referenceLookup = await _unitReader.GetGameUnitReferencesAsync(unitIds);
        ...
        foreach (var (patchFile, analysis, entries) in patchData)
        {
            ...
            foreach (var unit in analysis.UnitDetails)
            {
                ...
                if (!await VersionCheckService.ReadAtAsync(stream, checked((long)entry.Toc.TocOffset), unitData))
                {
                ...
                var currentMeshIds = GameUnitReferenceReader.ReadUnitMeshIds(unitData, unitData.Length);
    ...
    /// </summary>
    internal async Task<ModRepairResult> RepairModWithGameReferencesCoreAsync(
        DirectoryInfo modDirectory,
        Func<Task<AssistedModRepairPlan>> createPlanAsync)
    {
        ...
        try
        {
            ...
            var references = new Dictionary<long, GameUnitReferenceReader.GameUnitReferenceData>();
            if (plan.Actions.Count > 0)
            {
                var referenceLookup = await _unitReader.GetGameUnitReferencesAsync(
        ...
        List<string> blockers)
    {
        ...
        for (var i = 0; i < numFiles; i++)
        {
            ...
            if (!await VersionCheckService.ReadAtAsync(stream, tableOffset, buffer))
                return null;

            entries.Add(new AssistedPatchEntry(
                new VersionCheckService.PatchTocEntry(
        ...
        List<string> blockers)
    {
        ...
        foreach (var entry in entries.Where(entry => entry.Toc.TypeId == MaterialTypeId))
        {
            ...
            if (!await VersionCheckService.ReadAtAsync(stream, checked((long)entry.Toc.TocOffset), materialData))
            {
    ...
    private async Task RebuildPatchWithGameReferencesAsync(
        ...
        IReadOnlyDictionary<long, GameUnitReferenceReader.GameUnitReferenceData> references)
    {
    ...
    private static bool TryBuildUpdatedUnitData(
        byte[] originalData,
        GameUnitReferenceReader.GameUnitReferenceData reference,
        AssistedLodStrategy lodStrategy,
        out byte[] updatedData,
        out string error)
    {
