...
/// </summary>
internal sealed partial class VersionCheckRepairService
{
    ...
    public VersionCheckRepairService(VersionCheckService analysis, GameUnitReferenceReader unitReader, ILogger logger, LocalizationService localizationService)
    {
        _analysis = analysis;
    ...
    private readonly record struct RepairTocEntry(
        VersionCheckService.PatchTocEntry Toc,
        long TableOffset);
    ...
    {
        await VersionCheckShared.RepairGate.WaitAsync();
        try
        {
    ...
    /// </summary>
    internal async Task<ModRepairResult> RepairModCoreAsync(DirectoryInfo modDirectory)
    {
        ...
        try
        {
            ...
            foreach (var fileGroup in plan.Actions.GroupBy(a => a.PatchFilePath, StringComparer.OrdinalIgnoreCase))
            {
                ...
                var validation = await _analysis.AnalyzeSinglePatchFileStructureAsync(
        ...
        FileInfo? companionSource = null)
    {
        try
        {
            await using var stream = VersionCheckService.OpenPatchReadStream(patchFile);
            var header = new byte[HeaderSize];
            if (!await VersionCheckService.ReadAtAsync(stream, 0, header) ||
            ...
            for (var i = 0; i < numTypes; i++)
            {
                ...
                if (!await VersionCheckService.ReadAtAsync(stream, typeOffset, typeBuffer))
                {
            ...
            for (var i = 0; i < numFiles; i++)
            {
                ...
                if (!await VersionCheckService.ReadAtAsync(stream, tableOffset, entryBuffer))
                {
                    AddRepairBlocker(blockers, patchFile, "file table cannot be read");
                    return;
                }

                var toc = new VersionCheckService.PatchTocEntry(
            ...
            VersionCheckService.ValidateCompanionFiles(companionSource ?? patchFile, virtualEntries, companionAnalysis);
            ...
            VersionCheckService.ValidateMainDataRanges(virtualEntries, stream.Length, tableEnd, repairedRangeAnalysis);
    ...
    private async Task AddUnitSizeRepairsAsync(
        ...
        VersionCheckService.PatchTocEntry[] virtualEntries,
        FileStream stream,
        List<PatchRepairAction> actions,
        List<string> blockers,
        bool deferInvalidUnitHeaders)
    {
        for (var i = 0; i < entries.Count; i++)
        {
            ...
            var detail = await VersionCheckService.AnalyzeUnitResourceDeepAsync(
            ...
            var repairedDetail = await VersionCheckService.AnalyzeUnitResourceDeepAsync(
        ...
        List<string> blockers)
    {
        ...
        for (var i = 0; i < virtualEntries.Length; i++)
        {
            ...
            if (!VersionCheckService.IsRangeInBounds(entry.TocOffset, entry.TocSize, fileLength) ||
