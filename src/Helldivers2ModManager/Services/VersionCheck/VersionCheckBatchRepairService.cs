...
internal sealed class VersionCheckBatchRepairService
{
    ...
    public VersionCheckBatchRepairService(VersionCheckService analysis, VersionCheckRepairService repairService, VersionCheckCompanionRecoveryService companionService, ILogger logger, LocalizationService localizationService)
    {
        _analysis = analysis;
    ...
    /// 调用方必须已持有 VersionCheckShared.RepairGate；内部使用不加锁的 Core 变体，仅操作该模组目录。
    /// </summary>
        ...
        IProgress<BatchModRepairItem>? progress)
    {
        ...
        try
        {
            ...
            var companionPlan = await _companionService.CreateCompanionRecoveryPlanAsync(directory, cancellationToken);
            if (companionPlan.MissingCount > 0)
            {
                ...
                var companionResult = await _companionService.RecoverCompanionFilesCoreAsync(directory, cancellationToken);
            ...
            var metadataPlan = await _repairService.CreateRepairPlanAsync(directory);
            if (metadataPlan.ActionCount > 0 || metadataPlan.BlockingReasons.Count > 0)
            {
                ...
                var metadataResult = await _repairService.RepairModCoreAsync(directory);
            ...
            var assistedPlan = await _repairService.CreateAutomaticAssistedRepairPlanAsync(directory);
            if (assistedPlan.CanRepair)
            {
                var assistedResult = await _repairService.RepairModWithGameReferencesCoreAsync(
                    directory,
                    () => _repairService.CreateAutomaticAssistedRepairPlanAsync(directory));
    ...
    {
        var analysis = await _analysis.AnalyzeModPatchFilesAsync(directory);
