...
internal sealed class DatabaseService : IDisposable
{
	...
	// 按存储目录缓存连接串：不同 storageDirectory 各自初始化各自的库，
	// 避免"首次调用把路径钉死、后续传参被静默忽略打到第一个库上"的陷阱。
	...
	/// <summary>
	/// 是否已初始化（至少完成过一个存储目录的初始化）
	/// </summary>
	public bool IsInitialized => _connectionStrings.Count > 0;
	...
	{
		var connectionString = EnsureInitialized(storageDirectory);
	...
	/// <summary>
	/// 确保指定存储目录的数据库文件存在、WAL 模式已启用、表结构已创建。按目录各自执行一次。
	/// </summary>
	private string EnsureInitialized(string storageDirectory)
	{
		lock (_initLock)
		{
			if (_connectionStrings.TryGetValue(storageDirectory, out var existing))
				return existing;
			...
			try
			{
				...
				_connectionStrings[storageDirectory] = connectionString;
	...
	/// <summary>
	/// 释放资源：对所有已初始化的数据库执行 WAL checkpoint，确保数据持久化
	/// </summary>
	public void Dispose()
	{
