...
internal sealed partial class ManifestEditPageViewModel : ManifestEditorViewModel
{
    ...
    public ObservableCollection<CreateModOptionViewModel> EditOptions => EditorOptions;

    protected override string IconBaseDirectory => EditMod?.Data.Directory.FullName ?? string.Empty;

    protected override string BrowseIconTitle => _localizationService["ManifestEditPage.BrowseIconDialog"];

    protected override System.Windows.Media.ImageSource? FallbackIcon => EditMod?.Icon;
    ...
    public ManifestEditPageViewModel(ILogger<ManifestEditPageViewModel> logger,
        ...
        LocalizationService localizationService, IDialogService dialogService)
    ...
    public void InitializeFromMod()
    {
        ...
        if (IsV1Manifest && EditMod.Options is not null)
        {
            ...
            foreach (var opt in manifest.Options ?? [])
            {
                ...
                {
                    var subVm = new CreateModSubOptionViewModel(_localizationService, DialogService)
                    {
                        Name = sub.Name,
        ...
        else if (IsLegacyManifest)
        {
            ...
            if (legacyOptions is not null)
            {
                ...
                {
                    EditOptions.Add(new CreateModOptionViewModel(_localizationService, DialogService)
                    {
                        Name = optName,
            ...
            else
            {
                ...
                if (IconPath != currentIconPath)
                {
                    ...
                    else
                    {
                        ...
                        var sourcePath = BrowsedIconSourcePath ?? (Path.IsPathRooted(IconPath) ? IconPath : Path.Combine(modDir, IconPath));
                        CopyImageIfNeeded(sourcePath, modDir);
                        BrowsedIconSourcePath = null;
